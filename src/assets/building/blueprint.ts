...
function buildingMetrics(sketch: Sketch, recipe: RasterBuildingRecipe): BuildingMetrics {
  ...
  const roofY = MARGIN + ({
    ...
    shed: 38,
    mansard: 60,
  } as const)[recipe.identity.roof];
  return Object.freeze({
    left,
    right,
    bottom,
    roofY,
    cellWidth: (right - left) / recipe.identity.columns,
    cellHeight: (bottom - roofY) / recipe.identity.floors,
  });
}
...
function drawWindow(
  ...
  medium.tone(sketch, window, {
    ...
    color: lit ? recipe.identity.palette.accent : recipe.identity.palette.wall,
  });
...
function drawChimney(
  ...
  if (!recipe.identity.chimney.present) return;
  const chimneyWidth = Math.min(34, (right - left) * 0.1);
  const centerX = recipe.identity.chimney.side === 'left'
    ? left + (right - left) * 0.22
    : right - (right - left) * 0.22;
  const topY = Math.max(MARGIN - 2, roofY - recipe.identity.chimney.height * PIXELS_PER_UNIT);
...
function drawRoof(
  ...
  if (recipe.identity.roof === 'flat') {
  ...
  } else if (recipe.identity.roof === 'shed') {
    roof = [[left - 6, roofY - 24], [right + 6, roofY + 5]];
  } else if (recipe.identity.roof === 'mansard') {
    roof = [
      [left - 6, roofY + 5],
    ...
    const peakX = recipe.identity.roof === 'crooked' ? sketch.width * 0.63 : sketch.width * 0.5;
...
function drawBalconies(
  ...
  for (const balcony of recipe.identity.balconies) {
    const row = recipe.identity.floors - 1 - balcony.floorFromGround;
...
function drawBuildingShell(
  ...
  for (let row = 0; row < recipe.identity.floors; row += 1) {
    for (let column = 0; column < recipe.identity.columns; column += 1) {
      if (row === recipe.identity.floors - 1 && column === recipe.identity.door.column) continue;
      const centerX = left + cellWidth * (column + 0.5);
      const centerY = roofY + cellHeight * (row + 0.52);
      const lit = ((row * 17 + column * 31 + recipe.identity.seed) & 3) === 0;
      drawWindow(
        sketch,
        medium,
        centerX,
...
function drawDoor(
  ...
  const metrics = buildingMetrics(sketch, recipe);
  const centerX = metrics.left + metrics.cellWidth * (recipe.identity.door.column + 0.5);
  ...
  const opening = doorPolygon(
    ...
    width,
    height,
    recipe.identity.door.style === 'arched',
  );
  medium.tone(sketch, opening, { style: 'black', color: recipe.identity.palette.wall });
...
export function createRasterBuildingBlueprint(recipe: RasterBuildingRecipe): AssetBlueprint {
  ...
  const cellWidth = identity.width / identity.columns;
  const doorX = -identity.width / 2 + cellWidth * (identity.door.column + 0.5);
