...
- Name actions for player intent, never for a destructive destination. Map
  `B`/`Escape` to contextual `back` and `Menu`/`P` to `pause`; let explicit
  product state resolve close, resume, pause, or no-op. Do not map a physical
  control directly to `openMainMenu`. Keep locomotion vocabulary distinct from
  UI intent (`reverse` is not `back`).
- Make menus and focus-based UI consume the same abstract snapshot. Use normalized
  ...
  keep DOM focus and click behavior in the product. Reuse that navigator across
  setup menus, configurators, pause, and recoverable errors instead of adding a
  parallel gamepad-button path beside the schema.
- Make controller-driven DOM focus visually explicit with product-owned state;
  ...
  committed. Use native modal dialogs for focus containment and restore focus to
  gameplay on close. Do not scan the DOM every frame to maintain the marker.
...
- Track the latest meaningful browser device in the platform adapter when dynamic
  hints are required. Treat it as presentation metadata only: preserve gameplay
  behavior across devices, retain the last device while idle, and ignore deadzoned
  gamepad noise. Render hints from one binding-adjacent presentation table rather
  than scattering keyboard and gamepad labels through product markup.
