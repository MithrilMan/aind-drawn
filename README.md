...
[![Paper Circuit main menu in Graphite](experiments/landing/public/assets/paper-circuit/paper-circuit-menu-graphite-2026-readme-1200x750.webp)](https://mithrilman.github.io/aind-drawn/doodle-racing/)

| Sepia starting grid | Graphite grandstand exploration |
| --- | --- |
| [![Four procedural cars on the Paper Circuit starting grid in Sepia](experiments/landing/public/assets/paper-circuit/paper-circuit-race-sepia-2026-readme-1200x750.webp)](https://mithrilman.github.io/aind-drawn/doodle-racing/) | [![A procedural explorer and crowd in the Paper Circuit grandstand in Graphite](experiments/landing/public/assets/paper-circuit/paper-circuit-explore-graphite-2026-readme-1200x750.webp)](https://mithrilman.github.io/aind-drawn/doodle-racing/) |
| Race four procedural vehicles through a populated Doodle 3D circuit with arcade handling, drifting, race direction, telemetry, and audio. | Leave the car, walk through the paddock and grandstand, meet animated spectators, and operate semantic vehicle parts. |

[Projection Studio](https://mithrilman.github.io/aind-drawn/projection-studio/) complements the game
with a controlled inspection surface for the same identity across Drawn 2D, Doodle 3D, and smooth
solid projections.
...
- A versioned inked-solid compiler that packs every immutable asset registration into one skinned
  carrier and writes four semantic buffers in a single WebGL2 MRT pass.
- Scene-owned geometry and surface caches, explicit dynamic-geometry fallback, mobile tessellation
  policy, and lazy renderer lifecycle.
...
[raster rendering and portability](docs/raster-rendering.md). The packed carrier, cache identity,
and dynamic-geometry boundary are documented in
[compiled rendering](docs/compiled-rendering.md).
...
- a compiled MRT renderer that replaces thousands of per-part submissions with a handful of
  visible carriers while preserving articulation and semantic strokes;
- a menu that owns no hidden race scene: the stage and auxiliary previews are created only when
  the player asks for them and released on return.
...
- `src/projections/inked-solid` — Doodle 3D policy, compiled carriers, MRT pass, and compositor.
- `src/runtime` — sprite and solid rigs, shared geometry/surface resources, workers, and animation.
...
- `docs` — architecture, rendering, roadmap, and asset-authoring notes.
...
<sub>Origin note: the first creative spark came from studying
[KinderGrimm](https://github.com/albertobeiz/kindergrimm) by
[Alberto Beiz](https://github.com/albertobeiz). AIND Drawn subsequently evolved into an independent
architecture and implementation.</sub>
